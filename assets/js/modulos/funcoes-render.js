  ...
  if(!isEmpty(txt)){
    ...
    if(!isEmpty(proponentes) && proponentes.length > 0){
      proponentes.forEach((proponente, index) => {
        ...
        if(elementos.some('')){
          pendencias.push({proponente: `PROPONENTE ${index + 1} - ${elementos.valores.nome == undefined ? elementos.valores.nome.toUpperCase() : ''}`, pendente: elementos.search('').join().replaceAll('_', ' ').toUpperCase().replaceAll(',', '\n')})
        }
